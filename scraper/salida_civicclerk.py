...
def _requests_candidates(entry_url: str) -> List[Dict]:
    """
    Very light HTML scan (useful if the page outputs anchors server-side).
    """
    out: List[Dict] = []
    try:
...
def _playwright_candidates(entry_url: str) -> List[Dict]:
    """
    Use a quick tile/listing scan with Playwright to extract event links.
    """
    out: List[Dict] = []
...
def find_agenda_pdf(source_url: str, soup: Optional[BeautifulSoup] = None) -> Optional[str]:
    ...
    try:
        ...
        if not m:
            ...
            m = re.search(r"/files/agenda/(\d+)", soup.decode() if hasattr(soup, "decode") else str(soup))
